...
def processar_formulario(*args):
    ...
    "horario_atividades",
    "nome_supervisor", "formacao_supervisor", "registro_conselho"  # ← campos adicionados
    ]
    ...
    # Dicionário com nomes obrigatórios + labels
    campos_obrigatorios = {
    ...
    "especificacao_auxilio": "Especificação do Auxílio Transporte",
    "contraprestacao": "Contraprestação de Serviços",
    "especificacao_contraprestacao": "Especificação da Contraprestação",
    ...
    # Faz a validação apenas dos obrigatórios
    for i in indices_obrigatorios:
        nome = nomes_completos[i]
        valor = args[i]
        if not valor:
    ...
    if auxilio_transporte == "Sim" and not especificacao_auxilio:
        gr.Warning("⚠️ O campo 'Especificação do Auxílio Transporte' é obrigatório para a opção Sim.")
        return [gr.update() for _ in args]

    if contraprestacao == "Sim" and not especificacao_contraprestacao:
...
with gr.Blocks(theme="default") as demo:
    ...
    with gr.Row():
        ...
        cep_estudante = gr.Text(label="CEP (00000-00)*", placeholder="Ex: 12345-000")
